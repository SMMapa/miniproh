...
from pyhindsight.utils import format_meta_output, format_plugin_output
from fit.controller.entire_website import EntireWebsite as EntireWebsiteController
...
def parse_arguments():
    ...
    parser.add_argument('-f', '--format', choices=['xlsx', 'sqlite','json'],
...
def run_fit(output):
    unique_urls = []
    path = output + ".sqlite"
    connSelect = sqlite3.connect(path)
    getUrls = "SELECT DISTINCT url FROM timeline WHERE type='url'"
    connCursSelect = connSelect.execute(getUrls)
    for row in connCursSelect:
...
def run_hindsight(args):
    ...
    analysis_session.output_name = 'fitsight'
